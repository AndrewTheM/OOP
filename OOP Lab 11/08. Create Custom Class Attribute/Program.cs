...
namespace InfernoInfinity
{
    class Program
    {
            ...
            while (true)
            {
                try
                {
                    ...
                    switch (command)
                    {
                        ...
                        case "Author":
                            {
                                var infoAttr = GetWeaponClassInformation();
                                Console.WriteLine($"Author: {infoAttr.Author}");
                                break;
                            }
                        case "Revision":
                            {
                                var infoAttr = GetWeaponClassInformation();
                                Console.WriteLine($"Revision: {infoAttr.Revision}");
                                break;
                            }
                        case "Description":
                            {
                                var infoAttr = GetWeaponClassInformation();
                                Console.WriteLine($"Description: {infoAttr.Description}");
                                break;
                            }
                        case "Reviewers":
                            {
                                var infoAttr = GetWeaponClassInformation();
                                string joinedReviewers = string.Join(", ", infoAttr.Reviewers);
                                Console.WriteLine($"Reviewers: {joinedReviewers}");
                                break;
                            }
                        default:
        ...
        static InformationAttribute GetWeaponClassInformation()
        {
            var weaponAttrs = typeof(Weapon).GetCustomAttributes(typeof(InformationAttribute), false);
            return (InformationAttribute)weaponAttrs.SingleOrDefault();
        }
    }
}
